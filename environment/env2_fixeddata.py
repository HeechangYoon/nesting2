...
class HiNEST(object):
    def __init__(self, look_ahead=5,  # 상태 구성 시 포함할 부재의 수
        ...
        self.init_plate, self.init_part_list = generate_data(self.raw_part_list,
                                                             self.plate_l_min,
                                                             self.plate_l_max,
                                                             self.plate_b_min,
                                                             self.plate_b_max
                                                             )
        plate = copy.copy(self.init_plate)
        part_list = copy.copy(self.init_part_list)
    ...
    def _calculate_reward(self, done, overlap, ref_point):
        ...
        ref_end_row, ref_end_col = ref_point[0], ref_point[1]
